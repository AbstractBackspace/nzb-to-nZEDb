...
// Log file variables
$logger->log("TMDB_KEY: " . trim(file_get_contents('/etc/tmdb.key')), "DEBUG");
$logger->log("SALT_KEY: " . trim(file_get_contents('/etc/salt.key')), "DEBUG");
$logger->log("TOR_PROXY_HOST: " . trim(file_get_contents('/etc/tor.proxy.host')), "DEBUG");
...
function getIMDBData($imdbid) {
  ...
  $tmdbKey = trim(file_get_contents("/etc/tmdb.key"));
  $logger->log("Requesting Name from themoviedb with tmdbKey: " . $tmdbKey . " and imdbid: " . $imdbid);

  $url  = "http://api.themoviedb.org/3/movie/tt".$imdbid."?api_key=".$tmdbKey."&append_to_response=alternative_titles&language=de";
  $url  = "http://api.themoviedb.org/3/movie/tt".$imdbid."?api_key=".$tmdbKey."&language=de";
